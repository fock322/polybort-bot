...
function runBacktest(markets, tradesByMarket, klines, config) {
  ...
  for (const market of sortedMarkets) {
    ...
    // ── v4 IMPROVEMENT: Use VWAP of recent trades for entry signal ──
    // Don't rely on first trade price — use volume-weighted average of first ~2 min
    const earlyCutoff = marketStartTs + (config.earlyWindowSec || 120);
    const recentTrades = marketTrades.filter(t => t.timestamp < earlyCutoff);
    const upRecent = recentTrades.filter(t => t.outcome === "Up");
    const downRecent = recentTrades.filter(t => t.outcome === "Down");
    const vwapUp = upRecent.length > 0
      ? upRecent.reduce((s, t) => s + t.price * t.size, 0) / upRecent.reduce((s, t) => s + t.size, 0)
      : realUpMid;
    const vwapDown = downRecent.length > 0
      ? downRecent.reduce((s, t) => s + t.price * t.size, 0) / downRecent.reduce((s, t) => s + t.size, 0)
      : 1 - realUpMid;

    // v4: use VWAP for entry pricing instead of first trade
    const pUp = clamp(tickRound(vwapUp + modelSignal), TICK_SIZE, 1 - TICK_SIZE);
    ...
    // ── v4 IMPROVEMENT: Skip if market has already moved decisively ──
    // Check first 2 minutes of trades — if any side reached > 0.65, market is too one-sided
    if (config.skipDecidedMarkets !== false) {
      const earlyTrades = marketTrades.filter(t => t.timestamp < earlyCutoff);
      const earlyUpMax = Math.max(0, ...earlyTrades.filter(t => t.outcome === "Up").map(t => t.price));
      const earlyDownMax = Math.max(0, ...earlyTrades.filter(t => t.outcome === "Down").map(t => t.price));
      const decidedThreshold = config.decidedThreshold || 0.65;
      if (earlyUpMax > decidedThreshold || earlyDownMax > decidedThreshold) continue;
    }
    ...
    // ── v4 IMPROVEMENT: Paired hedge (avoid stuck losing side) ──
    // When ASK_UP fills (we sold UP), if we still hold DOWN position, the market is going UP.
    // DOWN side becomes illiquid — can't sell DOWN. Instead, BUY UP to hedge.
    // Math: hold DOWN @ entry, buy UP @ hedgePrice. If UP wins: 1 - hedge - entry per share.
    // Without hedge: 0 - entry per share (full loss). Hedge always helps if hedgePrice < 1.
    function pairedHedge(filledSide, fillTimestamp, fillPrice) {
      const otherSide = filledSide === "UP" ? "DOWN" : "UP";
      const otherKey = `${market.conditionId}_${otherSide}`;
      const otherPos = positions.get(otherKey);
      if (!otherPos || otherPos.quantity <= 0) return;

      // v4: Only hedge if the OTHER side's ASK won't fill in the remaining window
      // Look at all OTHER-side trades after fillTimestamp with price >= askOther
      // (If those trades exist, ASK would fill naturally — no need to hedge)
      const askOther = otherSide === "UP" ? askUp : askDown;
      const otherOutcome = otherSide === "UP" ? "Up" : "Down";
      const naturalExitTrades = marketTrades.filter(
        t => t.outcome === otherOutcome &&
        t.price >= askOther &&
        t.timestamp >= fillTimestamp &&
        t.timestamp < normalExitTs,
      );
      if (naturalExitTrades.length > 0) {
        if (config.debugPaired) console.error(`[pairedHedge] SKIP ${market.slug} - ${otherSide} ASK would fill naturally (${naturalExitTrades.length} trades >= ${askOther})`);
        return;  // ASK will fill naturally, no need to hedge
      }

      // Buy the FILLED side (the winning side) to hedge the OTHER side
      // We need filled-side trades at or after fillTimestamp
      // Note: outcome in DB is "Up"/"Down" (capitalized), normalize for comparison
      const filledOutcome = filledSide === "UP" ? "Up" : "Down";
      const hedgeTrades = marketTrades.filter(
        t => t.outcome === filledOutcome &&
        t.timestamp >= fillTimestamp &&
        t.timestamp < marketEndTs,
      );
      if (config.debugPaired) {
        const upTradesTotal = marketTrades.filter(t => t.outcome === filledOutcome).length;
        const upTradesAfterFill = marketTrades.filter(t => t.outcome === filledOutcome && t.timestamp >= fillTimestamp).length;
        console.error(`[pairedHedge] ${market.slug} filled=${filledSide} other=${otherSide} otherQty=${otherPos.quantity} fillTs=${fillTimestamp} marketEndTs=${marketEndTs} totalMarketTrades=${marketTrades.length} ${filledOutcome}Total=${upTradesTotal} ${filledOutcome}AfterFill=${upTradesAfterFill} inWindow=${hedgeTrades.length}`);
      }
      if (hedgeTrades.length === 0) return;

      // Use the FIRST available trade (earliest possible hedge, cheapest price)
      const hedgeTrade = hedgeTrades[0];
      const hedgePrice = tickRound(hedgeTrade.price);
      // v4 safety: only hedge if it locks in a profit (or break-even)
      // Math: hold OTHER @ entryOther, buy FILLED @ hedgePrice. Settle FILLED wins:
      //   Net = 1 - hedgePrice - entryOther per share
      // Require: hedgePrice + otherPos.entryPrice < 1 (after fee buffer)
      const breakEvenHedgePrice = 1 - otherPos.entryPrice - 0.02;  // 2% buffer for fees
      if (hedgePrice > breakEvenHedgePrice) {
        if (config.debugPaired) console.error(`[pairedHedge] SKIP ${market.slug} hedgePrice=${hedgePrice} > breakEven=${breakEvenHedgePrice.toFixed(3)} (entryOther=${otherPos.entryPrice})`);
        return;
      }
      const hedgeQty = Math.min(otherPos.quantity, config.maxPositionSize);
      const hedgeFee = calcTakerFee(hedgeQty, hedgePrice, feeRate);
      const hedgeCost = hedgePrice * hedgeQty + hedgeFee;
      if (cash.balance < hedgeCost) return;

      cash.balance -= hedgeCost;
      totalFees += hedgeFee; takerTrades++;

      // Add to the FILLED side position (it's now a hedge)
      const hedgeKey = `${market.conditionId}_${filledSide}`;
      const existing = positions.get(hedgeKey);
      if (existing) {
        existing.quantity += hedgeQty;
        existing.costBasis += hedgeCost;
        existing.entryPrice = existing.costBasis / existing.quantity;
      } else {
        positions.set(hedgeKey, {
          side: filledSide, entryPrice: hedgePrice, quantity: hedgeQty,
          costBasis: hedgeCost, openedAt: hedgeTrade.timestamp, marketSlug: market.slug,
        });
      }
      inventory.set(market.conditionId, (inventory.get(market.conditionId) || 0) + (filledSide === "UP" ? hedgeQty : -hedgeQty));

      btTrades.push({
        ts: hedgeTrade.timestamp, slug: market.slug,
        side: `PAIRED_HEDGE_${filledSide}`, price: hedgePrice, qty: hedgeQty,
        fee: hedgeFee, rebate: 0, pnl: 0, reason: "paired_hedge", cash: cash.balance,
      });
    }
    ...
    if (upPos && upPos.quantity > 0 && askUp > 0) {
      ...
      if (upBuyTrades.length > 0) {
        ...
        // v4: paired hedge - if DOWN position still open, BUY UP to hedge it
        pairedHedge("UP", fillTrade.timestamp, fillPrice);
      }
    }
    ...
    if (downPos && downPos.quantity > 0 && askDown > 0) {
      ...
      if (downBuyTrades.length > 0) {
        ...
        // v4: paired hedge - if UP position still open, BUY DOWN to hedge it
        pairedHedge("DOWN", fillTrade.timestamp, fillPrice);
      }
    }
...
function main() {
  ...
  if (opts.sweep) {
    ...
    const config = {
      ...
      maxProbEntry: parseFloat(opts.maxprob) || 0.65,
      minProbEntry: parseFloat(opts.minprob) || 0.35,
      ...
      // v4 improvements
      skipDecidedMarkets: opts.skipdecided !== "false",
      earlyWindowSec: parseFloat(opts.earlySec) || 120,
      decidedThreshold: parseFloat(opts.decided) || 0.65,
      debugPaired: opts.debug === "true",
    };
    ...
    const fs = require("fs");
    const outPath = path.join(__dirname, "..", "download", "backtest_v4_result.json");
