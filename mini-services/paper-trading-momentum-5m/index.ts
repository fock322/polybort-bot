#!/usr/bin/env bun
/**
 * Paper Trading Mini-Service — MOMENTUM 5-min markets
 * Port: 3007
 *
 * Tests momentum strategy on 5-min BTC/ETH Up/Down markets.
 * Separate from 15-min momentum (port 3003).
 */
...
// Override config for 5-min markets
updateConfig({
  marketIntervalMin: 5,
  autoExitMinutes: 1,  // auto-exit at tau < 1 min for 5-min markets
});
console.log(`[momentum-5m] Market interval: 5 minutes`);
...
initCoinbaseWs();
console.log(`[paper-trading-momentum-5m:${PORT}] Coinbase WebSocket initialized (BTC-USD, ETH-USD, SOL-USD)`);
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (path === "/health" && method === "GET") {
        return Response.json({ status: "ok", service: "paper-trading-momentum-5m", port: PORT, strategy: "momentum", marketInterval: "5m", uptime: process.uptime() }, { headers: corsHeaders });
      }
...
console.log(`═══════════════════════════════════════════════════════════════`);
console.log(`  Paper Trading — MOMENTUM 5m strategy — Port ${PORT}`);
...
<title>Polymarket MM Bot — MOMENTUM 5m</title>
<style>
...
<body>
<div class="container">
  <h1>🚀 Polymarket MM Bot — MOMENTUM 5m <span class="strategy-badge" style="background:#f59e0b;">5-MIN MARKETS</span></h1>
  <p class="subtitle">BTC + ETH 5-min Up/Down • mid ≥ $0.80 • SL dynamic</p>
  ...
  <div class="refresh-info">Авто-обновление каждые 5 секунд • Momentum 5m • Port 3007</div>
</div>
<script>
