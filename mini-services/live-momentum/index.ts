#!/usr/bin/env bun
/**
 * LIVE Trading Mini-Service — MOMENTUM strategy
 * Standalone Bun HTTP server — independent of paper mode
 * Port: 3006
 *
 * ⚠️ LIVE MODE: real money trading on Polymarket
 * Separate from paper mode (ports 3003-3005) — does NOT share code.
 * Updates to paper mode don't affect this until explicitly deployed.
 */
...
// ⚠️ LIVE MODE: enable real money trading
// Override config to use live mode with $30 starting balance
updateConfig({
  liveMode: true,
  startingBalance: parseFloat(process.env.LIVE_STARTING_BALANCE || "30"),
});
console.log(`[LIVE-MOMENTUM] ⚠️ LIVE MODE ENABLED — starting balance: $${process.env.LIVE_STARTING_BALANCE || "30"}`);
...
initCoinbaseWs();
console.log(`[live-momentum:${PORT}] Coinbase WebSocket initialized (BTC-USD, ETH-USD, SOL-USD)`);
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (path === "/start" && method === "POST") {
        ...
        const btc = await getBtcPrice();
        const status = getStatus(btc);
        return Response.json({ success: true, message: "Momentum bot started", status }, { headers: corsHeaders });
      }
      ...
      if (path === "/health" && method === "GET") {
        return Response.json({ status: "ok", service: "live-momentum", port: PORT, strategy: "momentum", liveMode: true, uptime: process.uptime() }, { headers: corsHeaders });
      }
      ...
      console.error("[Momentum Service] Error:", e.message);
...
console.log(`═══════════════════════════════════════════════════════════════`);
console.log(`  ⚠️ LIVE Trading Mini-Service — MOMENTUM strategy — Port ${PORT}`);
...
<title>Polymarket MM Bot — LIVE MOMENTUM</title>
<style>
...
<body>
<div class="container">
  <h1>🚀 Polymarket MM Bot — LIVE MOMENTUM <span class="strategy-badge" style="background:#ef4444;">⚠️ REAL MONEY</span></h1>
  <p class="subtitle">Trailing TP (no ceiling) • SL 5% • Trend-following</p>
  ...
  <div class="refresh-info">Авто-обновление каждые 5 секунд • Momentum strategy • Port 3003</div>
</div>
<script>
...
function updateUI(d){
  ...
  document.getElementById('statusText').textContent=running?'🟢 Momentum бот работает':'🔴 Бот остановлен';
