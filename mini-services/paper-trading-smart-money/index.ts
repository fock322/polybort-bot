#!/usr/bin/env bun
/**
 * Paper Trading Mini-Service — SMART MONEY v2 strategy
 * Port: 3004
 *
 * Strategy: trend-following with strict confirmation + maker TP exit (0 fee)
 * - TP 8% via maker ASK (0 fee) → net win +$0.77
 * - SL 3% via taker (BTC-strike based) → net loss -$0.69
 * - Break-even WR: 47%, at 90% WR: EV = +$0.62/trade
 */
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (path === "/start" && method === "POST") {
        ...
        const btc = await getBtcPrice();
        const status = getStatus(btc);
        return Response.json({ success: true, message: "Smart Money bot started", status }, { headers: corsHeaders });
      }
      ...
      console.error("[SmartMoney Service] Error:", e.message);
...
console.log(`═══════════════════════════════════════════════════════════════`);
console.log(`  Paper Trading — SMART MONEY v2 — Port ${PORT}`);
console.log(`  Dashboard: http://localhost:${PORT}/dashboard`);
console.log(`  Strategy: trend-following + maker TP (0 fee) + BTC-strike SL`);
console.log(`  TP 8% (maker) / SL 3% (taker) | Position $10`);
console.log(`═══════════════════════════════════════════════════════════════`);
...
<title>Polymarket MM Bot — SMART MONEY v2</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .subtitle { color: #a855f7; font-size: 14px; margin-bottom: 20px; }
  ...
  .section-title { font-size: 14px; font-weight: 600; margin-bottom: 12px; color: #a855f7; }
  ...
  .strategy-badge { display: inline-block; background: #a855f7; color: #0f1117; padding: 4px 12px; border-radius: 4px; font-size: 12px; font-weight: 700; margin-left: 10px; }
...
<div class="container">
  ...
  <div class="refresh-info">Авто-обновление каждые 5 секунд • Smart Money v2 • Port 3004</div>
</div>
<script>
...
function updateUI(d){
  ...
  document.getElementById('statusText').textContent=running?'🟢 Smart Money работает':'🔴 Бот остановлен';
  ...
  if(d.positions&&d.positions.length>0){posEl.innerHTML=d.positions.map(p=>{
    ...
    const tp=p.tpThreshold||0;
    const tpReady=p.tpReady?' ✅ TP READY':' ❌ TP wait';
    return '<div class="row" style="flex-direction:column;align-items:stretch;padding:10px 0;">'+
      ...
      '</div>'+
      '<small style="color:#52525b;margin-top:4px;display:block;">bid $'+bid+' ask $'+ask+' mid $'+mid+' | TP>$'+tp.toFixed(4)+tpReady+'</small>'+
    '</div>';
  ...
  if(d.markets&&d.markets.length>0){mktEl.innerHTML=d.markets.map(m=>{
    ...
    if(s.should){
      ...
      sigHtml='<div style="margin-top:4px;font-size:12px;color:'+sc2+';">💎 SMART MONEY ENTER '+s.side+' (conf='+s.confidence+'/100)</div>';
    }else{
