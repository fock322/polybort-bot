#!/usr/bin/env bun
/**
 * LIVE Trading Mini-Service — MOMENTUM strategy on 5-MIN markets
 * Standalone Bun HTTP server — independent of paper mode and 15m live.
 * Port: 3008
 *
 * ⚠️ LIVE MODE: real money trading on Polymarket 5-min BTC/ETH Up/Down markets.
 *
 * Setup:
 *   - Reads CLOB_PRIVATE_KEY, CLOB_FUNDER_ADDRESS, LIVE_STARTING_BALANCE from .env
 *   - signatureType=3 (POLY_1271 / Privy proxy) when funder address is set
 *   - marketIntervalMin=5 → only fetches btc-updown-5m-* and eth-updown-5m-* slugs
 *   - SOL excluded (no 5-min SOL Up/Down markets on Polymarket)
 *
 * Strategy: Momentum v4.2 (with BTC trend filter — blocks entry against strong 1m move)
 */
...
// ⚠️ LIVE MODE: enable real money trading on 5-min markets
// Override config for 5-min market interval + live mode.
const startingBalance = parseFloat(process.env.LIVE_STARTING_BALANCE || "42");
updateConfig({
  liveMode: true,
  marketIntervalMin: 5,      // 5-min markets only (btc-updown-5m-*, eth-updown-5m-*)
  autoExitMinutes: 1,         // auto-exit at tau < 1 min for 5-min markets
  startingBalance,
});

// Sanity check: verify CLOB creds are present BEFORE announcing live mode.
const pk = process.env.CLOB_PRIVATE_KEY || process.env.NEXT_PUBLIC_CLOB_PRIVATE_KEY;
const funder = process.env.CLOB_FUNDER_ADDRESS || process.env.NEXT_PUBLIC_CLOB_FUNDER_ADDRESS;
if (!pk) {
  console.error("╔════════════════════════════════════════════════════════════════╗");
  console.error("║  ⛔ FATAL: CLOB_PRIVATE_KEY not set in environment!            ║");
  console.error("║  Service will fall back to PAPER mode silently.                ║");
  console.error("║  Add to .env:  CLOB_PRIVATE_KEY=0x...                          ║");
  console.error("╚════════════════════════════════════════════════════════════════╝");
  process.exit(1);
}
console.log(`[LIVE-MOMENTUM-5M] ⚠️ LIVE MODE — 5-min BTC/ETH markets`);
console.log(`[LIVE-MOMENTUM-5M]   startingBalance: $${startingBalance}`);
console.log(`[LIVE-MOMENTUM-5M]   funder (proxy):  ${funder || "<none — will use EOA>"}`);
console.log(`[LIVE-MOMENTUM-5M]   signatureType:   ${funder ? "3 (POLY_1271 proxy)" : "0 (EOA direct)"}`);
...
initCoinbaseWs();
console.log(`[live-momentum-5m:${PORT}] Coinbase WebSocket initialized (BTC-USD, ETH-USD, SOL-USD)`);
  ...
  async fetch(req) {
    ...
    try {
      if (path === "/" && method === "GET") {
        ...
        return Response.json({ ...status, markets: markets.slice(0, 6), positions: positions.slice(0, 10) }, { headers: corsHeaders });
      }
      ...
      if (path === "/start" && method === "POST") {
        ...
        const btc = await getBtcPrice();
        const status = getStatus(btc);
        return Response.json({ success: true, message: "Live momentum-5m bot started", status }, { headers: corsHeaders });
      }
      ...
      if (path === "/reset" && method === "POST") {
        resetEngine();
        setStrategy("momentum");  // re-set after reset
        updateConfig({
          liveMode: true,
          marketIntervalMin: 5,
          autoExitMinutes: 1,
      ...
      console.error("[Live Momentum-5m Service] Error:", e.message);
...
console.log(`═══════════════════════════════════════════════════════════════`);
console.log(`  ⚠️ LIVE Trading — MOMENTUM 5m — Port ${PORT}`);
console.log(`  Dashboard: http://localhost:${PORT}/dashboard`);
console.log(`  Markets:   5-min BTC + ETH Up/Down (SOL excluded)`);
console.log(`  Strategy:  Momentum v4.2 (with BTC trend filter)`);
console.log(`═══════════════════════════════════════════════════════════════`);
...
<title>Polymarket — LIVE MOMENTUM 5m</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .subtitle { color: #ef4444; font-size: 14px; margin-bottom: 20px; }
  ...
  .section-title { font-size: 14px; font-weight: 600; margin-bottom: 12px; color: #ef4444; }
  ...
  .strategy-badge { display: inline-block; background: #ef4444; color: #fff; padding: 4px 12px; border-radius: 4px; font-size: 12px; font-weight: 700; margin-left: 10px; }
  .market-badge { display: inline-block; background: #f59e0b; color: #0f1117; padding: 4px 12px; border-radius: 4px; font-size: 12px; font-weight: 700; margin-left: 6px; }
  @media (max-width:600px) { .cards { grid-template-columns:1fr; } }
</style>
</head>
<body>
<div class="container">
  <h1>🚀 Polymarket — LIVE MOMENTUM 5m <span class="strategy-badge">⚠️ REAL MONEY</span><span class="market-badge">5-MIN MARKETS</span></h1>
  <p class="subtitle">BTC + ETH 5-min Up/Down • Momentum v4.2 (BTC trend filter) • FAK orders</p>
  ...
  <div class="section"><div class="section-title">🏪 Активные рынки (5-min)</div><div id="markets"><div class="empty">Нет рынков</div></div></div>
  ...
  <div class="refresh-info">Авто-обновление каждые 5 секунд • LIVE MOMENTUM 5m • Port 3008</div>
</div>
<script>
...
function updateUI(d){
  ...
  document.getElementById('statusText').textContent=running?'🟢 LIVE бот работает (real money)':'🔴 Бот остановлен';
  ...
  if(d.positions&&d.positions.length>0){posEl.innerHTML=d.positions.map(p=>{
    ...
    return '<div class="row" style="flex-direction:column;align-items:stretch;padding:10px 0;">'+
      ...
      '</div>'+
      '<small style="color:#52525b;margin-top:4px;display:block;">bid $'+bid+' ask $'+ask+' mid $'+mid+' | peak=$'+peak.toFixed(2)+'</small>'+
    '</div>';
  ...
  if(d.markets&&d.markets.length>0){mktEl.innerHTML=d.markets.map(m=>{
    ...
    const tc=ml<2?'#ef4444':ml<4?'#f59e0b':'#71717a';
    const s=m.smartSignal||{};
    let sigHtml='';
    if(s.should){
      const sc2=s.confidence>=80?'#22c55e':s.confidence>=60?'#f59e0b':'#ef4444';
      sigHtml='<div style="margin-top:4px;font-size:12px;color:'+sc2+';">🎯 MOMENTUM ENTER '+s.side+' (conf='+s.confidence+'/100)</div>';
    }else{
      sigHtml='<div style="margin-top:4px;font-size:12px;color:#71717a;">⏸ '+((s.reason||'').substring(0,90))+'</div>';
    }
    return '<div class="row" style="flex-direction:column;align-items:stretch;padding:10px 0;">'+
      '<div style="display:flex;justify-content:space-between;">'+
...
async function fetchTrades(){try{const r=await fetch(api('/trades'));const d=await r.json();const t=d.trades||[];const el=document.getElementById('tradesList');if(t.length===0){el.innerHTML='<div class="empty">Нет сделок</div>';return;}el.innerHTML=t.slice(0,15).map(t=>{const s=t.side||'?';const p=(t.price||0).toFixed(2);const q=t.quantity||0;const rs=t.reason||'?';const pnl=t.pnl||0;const ps=pnl!==0?(pnl>0?'+':'')+'$'+pnl.toFixed(4):'—';const pc=pnl>0?'pnl-positive':pnl<0?'pnl-negative':'';const ts=new Date(t.executedAt||0).toLocaleTimeString('ru-RU');const ctx=t.context||{};const slug=ctx.marketSlug||'?';const entry=ctx.entryPrice?(ctx.entryPrice).toFixed(3):'?';const hold=ctx.holdTimeMs?(ctx.holdTimeMs/1000).toFixed(0)+'s':'?';const vol=ctx.marketVolume?'$'+ctx.marketVolume.toFixed(0):'?';const a1m=ctx.btcChange1m?(ctx.btcChange1m*100).toFixed(2)+'%':'?';const a5m=ctx.btcChange5m?(ctx.btcChange5m*100).toFixed(2)+'%':'?';const pnlSym=pnl>0?'🟢':pnl<0?'🔴':'⚪';const liveTag=t.isPaperTrade?'':' 🔴LIVE';return '<div class="row" style="flex-direction:column;align-items:stretch;padding:10px 0;">'+'<div style="display:flex;justify-content:space-between;">'+'<span class="row-label">'+pnlSym+' '+ts+' • '+s+' '+q+'@$'+p+' <small>('+rs+')</small>'+liveTag+'</span>'+'<span class="row-value '+pc+'">'+ps+'</span>'+'</div>'+'<small style="color:#71717a;margin-top:3px;display:block;">'+slug.substring(0,40)+' | entry=$'+entry+' hold='+hold+' vol='+vol+' 1m='+a1m+' 5m='+a5m+'</small>'+'</div>';}).join('');}catch(e){}}
